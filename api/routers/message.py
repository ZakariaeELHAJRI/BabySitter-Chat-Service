...
def update_message(message_id: int, message_data: dict, db: Session = Depends(get_db)):
    db_message = update_message(db=db, message_id=message_id, message_data=message_data)
    if db_message is None:
        raise HTTPException(status_code=404, detail="Message not found")
    return db_message
...
def delete_message(message_id: int, db: Session = Depends(get_db)):
    db_message = delete_message(db=db, message_id=message_id)
    if db_message is None:
        raise HTTPException(status_code=404, detail="Message not found")
    return db_message
...
def read_messages(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    messages = get_messages(db=db, skip=skip, limit=limit)
    return messages
...
def read_messages_by_conversation(conversation_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    messages = get_messages_by_conversation(db=db, conversation_id=conversation_id, skip=skip, limit=limit)
    return messages

@router.get("/messages/user/{user_id}")
def read_messages_by_user(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    messages = get_messages_by_user(db=db, user_id=user_id, skip=skip, limit=limit)
    return messages
...
def read_unread_messages_by_user(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    messages = get_unread_messages_by_user(db=db, user_id=user_id, skip=skip, limit=limit)
    return messages
...
def read_unread_messages_by_conversation(conversation_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    messages = get_unread_messages_by_conversation(db=db, conversation_id=conversation_id, skip=skip, limit=limit)
    return messages